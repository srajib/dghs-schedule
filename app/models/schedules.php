...
class Schedules extends MY_Model
{
    ...
    public function save(array $data)
    {
        ...
        if(empty($data['to_date'])) {
            $data['to_date'] = DateHelper::humanToMysql($data['date']);
        } else {
            $data['to_date'] = DateHelper::humanToMysql($data['to_date']);
        }
    ...
    public function getAllForPrint( $options = array(), $postData = null)
    {
        ...
        $this->db->orwhere("{$this->table}.to_date >=", date('Y-m-d'));
    ...
    protected function _setQueryParts($options = array())
    {
        ...
        if (empty ($options['isPast'])) {
            ...
            $this->db->where("{$this->table}.to_date <", date('Y-m-d'));
            $this->db->orWhere("{$this->table}.to_date <", date('Y-m-d'));
        }
    }
    ...
    public function getDetailById($id)
    {
        ...
        $this->db->select('S.schedule_id, S.title, S.description, S.date, S.time, S.venue, S.is_date_not_confirmed, S.to_date, S.to_time, S.is_time_not_confirmed, ST.title AS status, GS.title AS grace');
    ...
    public function pastschedule(array $options, $postData = null)
    {
        $fields = "{$this->table}.schedule_id,
                   ...
                   {$this->table}.is_time_not_confirmed,
                   GS.title AS grace,
                   GR.print_title AS group_title";

        $this->db->select($fields);
        $this->db->from($this->table);
        $this->db->join('grace_statuses AS GS', "GS.grace_status_id = {$this->table}.grace_status_id" );
        $this->db->join('groups AS GR', "GR.group_id = {$this->table}.group_id" );
