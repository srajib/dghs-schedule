<div class="block">
    ...
    <div class="block_content event-details">
        ...
        <span class="description"> <?php echo DateHelper::mysqlToHuman($schedule['date']);
                                         echo (!empty($schedule['to_date']) AND ($schedule['date'] != $schedule['to_date']))?'   To   '. DateHelper::mysqlToHuman($schedule['to_date']):''?>
            &nbsp;(<?php echo ( $schedule['is_date_not_confirmed'] == 0 ) ? 'Confirmed':'Not Confirmed' ?>)</span><br /><br />
        ...
        <span class="description"> <?php echo date('l', strtotime($schedule['date']));
                                         echo (!empty($schedule['to_date']) AND ($schedule['date'] != $schedule['to_date']))?'   To   '. date('l', strtotime($schedule['to_date'])):'';?></span><br /><br />

        <span class="title">Time </span>
        <span class="description"> <?php echo date("g:i a", STRTOTIME($schedule['time']));
                                         echo (!empty($schedule['to_date']) AND ($schedule['date'] != $schedule['to_date']))?'   To   '. date("g:i a", STRTOTIME($schedule['to_time'])):''; ?>&nbsp;(<?php echo
