<div class="block">
    ...
    <div class="block_content">
        ...
        <table cellpadding="0" cellspacing="0" width="100%">
            ...
            <tr>
                <td colspan="7" class="nodatamsg">No event is available.</td>
            </tr>

            <?php else : foreach($schedules AS $schedule) : ?>
                
            <tr>
                <td class="centered"><?php echo DateHelper::mysqlToHuman($schedule['date']);
                                           echo (!empty($schedule['to_date']) AND ($schedule['date'] != $schedule['to_date']))?'   To   '. DateHelper::mysqlToHuman($schedule['to_date']):'';
                                      ?>
                </td>
                <td class="centered"><?php echo date('l', strtotime($schedule['date']));
                                           echo (!empty($schedule['to_date']) AND ($schedule['date'] != $schedule['to_date']))?'   To   '. date('l', strtotime($schedule['to_date'])):'';
                                      ?>
                </td>
                <td class="centered"><?php echo date("g:i a", STRTOTIME($schedule['time']));
                                           echo ((!empty($schedule['to_time']) AND ($schedule['time'] != $schedule['to_time']))?'   To   '. date("g:i a", STRTOTIME($schedule['to_time'])):''); ?>
                </td>
